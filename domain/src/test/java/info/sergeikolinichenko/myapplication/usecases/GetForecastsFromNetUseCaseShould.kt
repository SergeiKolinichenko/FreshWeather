...
class GetForecastsFromNetUseCaseShould {
  ...
  // system under test
  private val systemUnderTest = GetForecastsFromNetUseCase(repository)

  @Test
  fun `get forecast from repository`(): Unit = runTest {
    // Act
    systemUnderTest.invoke(cities)
    // Assert
  ...
  fun `return forecast from repository`(): Unit = runTest {
    ...
    // Act
    val result = systemUnderTest.invoke(cities).getOrNull()
    // Assert
  ...
  fun `return error from repository`(): Unit = runTest {
    ...
    // Act
    val result = systemUnderTest.invoke(cities).exceptionOrNull()?.message
    // Assert
